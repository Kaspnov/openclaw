...
function inspectRuntimeToolInputSchema(
  tool: Pick<AnyAgentTool, "name" | "parameters">,
  toolIndex: number,
  ...
  const projection = projectRuntimeToolInputSchema(parametersRead.value, `${toolName}.parameters`);
...
function inspectRuntimeToolEntries<TTool extends Pick<AnyAgentTool, "name" | "parameters">>(
  ...
  for (const entry of entries) {
    ...
    const diagnostic = inspectRuntimeToolInputSchema(entry.tool, entry.toolIndex);
    if (diagnostic) {
...
export function inspectRuntimeToolInputSchemas(
  ...
  return inspectRuntimeToolEntries(readRuntimeToolEntries(tools)).diagnostics;
}
...
>(tools: readonly TTool[]): RuntimeToolSchemaInspection<TTool> {
  return inspectRuntimeToolEntries(readRuntimeToolEntries(tools));
}
